...
describe('avatar locale', () => {
  ...
  it('translates known labels and preserves unknown text', () => {
    ...
    expect(translateAvatarText('zh-Hans', 'Part position X')).toBe('部件 X 位置')
    expect(translateAvatarText('zh-Hans', 'Part rotation Z')).toBe('部件 Z 轴旋转')
    expect(translateAvatarText('zh-Hans', 'Part corner roundness')).toBe('部件圆角程度')
    expect(translateAvatarText('zh-Hans', 'Group offset Y')).toBe('分组 Y 偏移')
    ...
    expect(translateAvatarText('zh-Hans', 'More panel actions')).toBe('更多面板操作')
    expect(translateAvatarText('zh-Hans', 'Export avatar')).toBe('导出形象')
    expect(translateAvatarText('zh-Hans', 'More timeline options')).toBe('更多时间线操作')
    expect(translateAvatarText('zh-Hans', 'Clear selected track')).toBe('清空选中轨道')
