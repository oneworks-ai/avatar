...
Seed individual parameters without encoding the whole Avatar in the Seed. Supported bindings include palette, background style, camera background, cat-ear dimensions, face preset, coat-pattern controls, and `scene.view.pose`. A Seeded view uses one consistent moderate scale and lower composition, moves horizontally, and keeps its yaw and pitch close to the frame center with a small bounded variation. Roll remains zero, so rerolling never turns the Avatar upside down or around the screen plane:
