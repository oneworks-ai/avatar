...
Seed 只参与指定参数，不需要把整份 Avatar 配置编码进 Seed。支持的绑定包括配色、背景样式、相机背景、猫耳与狗耳尺寸、狗头宽高、面部预设、毛纹控制，以及 `scene.view.pose`。由 Seed 控制视图时会使用统一的适中大小与下沉构图，只随机左右位置；yaw 与 pitch 会在朝向画面中心的基础上做小幅受限变化，roll 始终为零，因此重新随机不会让形象倒置或在画面平面里转圈：
...
狗狗类型（柴犬、哈士奇、柯基、金毛、边牧和斑点狗）沿用同一套 definition 驱动的档案模型：自然材质、真实 3D 头部与耳朵几何，以及投影到表面的口鼻浅毛、额纹、面罩或可复现斑点都会保存在场景中；Seed 只会改变该品种明确允许的自然参数。每个犬种的头部宽度与高度均可独立调整，并在符合自身特征的区间内随机变化。
